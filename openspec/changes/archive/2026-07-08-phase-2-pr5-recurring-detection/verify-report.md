...
2. **Same-day fix** — three same-day transactions previously produced a meaningless rule with `period_label="weekly"` and `period_days=0`. The guard is now `if median_interval == 0: return None` AFTER computing the median, which catches both the all-zero case (3 same-day → intervals `[0, 0]`) and the majority-same-day case (3 on day D + 1 on day D+30 → intervals `[0, 0, 30]` → median 0). The earlier `all(i == 0 for i in intervals)` guard only caught the all-zero case. The majority-same-day case is locked by `test_majority_same_day_with_one_later_does_not_create_rule`.
3. **Relationship loading** — `RecurringRule.merchant` and `RecurringRule.transactions` were `lazy="joined"` and `lazy="selectin"` respectively. Both fields are not in the API response, so the default read paid for a JOIN and a second round-trip. Both changed to `lazy="noload"`. Callers that need the relationships must opt in at query time with `selectinload` / `joinedload` — `session.refresh` does NOT override `noload` on SQLAlchemy 2.0 and the relationship would stay empty.
