...
### Requirement: `RecurringRuleResponse.confidence` Sanitizes Non-Finite Values to `[0.0, 1.0]`

The `confidence` field in `RecurringRuleResponse` MUST be sanitized to the closed interval `[0.0, 1.0]` before Pydantic validation. Non-finite values (`NaN`, `+inf`, `-inf`) MUST be coerced to `0.0` rather than raising `ValidationError`. This is defensive — the detector's current algorithm never produces non-finite `confidence` values, but a future contributor or a corrupt database row must not crash the API surface. The sanitization MUST run via `field_validator("confidence", mode="before")` so it executes before the existing `Field(ge=0.0, le=1.0)` constraint check; the coerced `0.0` satisfies that bound, so no `ValidationError` is raised for any input.

#### Scenario: NaN confidence coerces to 0.0

- **GIVEN** a database row with `confidence = NaN`
- **WHEN** the row is serialized via `RecurringRuleResponse.from_attributes(row)`
- **THEN** the response carries `confidence = 0.0` (not `NaN`)
- **AND** no `ValidationError` is raised

#### Scenario: Positive infinity coerces to 0.0

- **GIVEN** a database row with `confidence = +inf`
- **WHEN** the row is serialized via `RecurringRuleResponse.from_attributes(row)`
- **THEN** the response carries `confidence = 0.0`
- **AND** no `ValidationError` is raised

#### Scenario: Negative infinity coerces to 0.0
...
#### Scenario: Finite out-of-range values are still clamped

- **GIVEN** a database row with `confidence = 1.5` or `confidence = -0.1`
- **WHEN** the row is serialized via `RecurringRuleResponse.from_attributes(row)`
- **THEN** the response carries `confidence` clamped to `[0.0, 1.0]` via the existing `Field(ge=0.0, le=1.0)` constraint
- **AND** the sanitization is a no-op for these finite values

#### Scenario: Direct construction with a finite value passes through unchanged

- **GIVEN** `RecurringRuleResponse(confidence=0.42, ...)` is constructed with a finite, in-bounds value
- **WHEN** Pydantic validation runs
- **THEN** the response carries `confidence = 0.42` (the validator is a no-op for finite values)

### Requirement: `recurring_rules` Has a UNIQUE Constraint on the `(merchant_id, amount_min, amount_max, currency, period_days)` Upsert Key

The `recurring_rules` table MUST have a UNIQUE constraint on the 5-column tuple `(merchant_id, amount_min, amount_max, currency, period_days)`. This enforces the upsert key at the database level so two concurrent detector runs cannot create duplicate rows. The constraint MUST be named `uq_recurring_rules_upsert_key` for predictability in migrations and error messages.

When the migration is applied to a database that already contains rows, the upgrade MUST deduplicate existing rows by `(merchant_id, amount_min, amount_max, currency, period_days)`: for each duplicate group, the row with the highest `confidence` is kept and the others are deleted. For tie-breaking, `last_seen_date` is set to `max(last_seen_date)` across the group. This matches the detector's "freshest data wins" intent — when a pattern re-fires, the existing rule is updated, not duplicated.

#### Scenario: Unique constraint is created on upgrade

- **GIVEN** a fresh database with no `recurring_rules` rows
- **WHEN** `alembic upgrade head` is run
- **THEN** the `recurring_rules` table has a UNIQUE constraint named `uq_recurring_rules_upsert_key` on `(merchant_id, amount_min, amount_max, currency, period_days)`

#### Scenario: Duplicate rows are deduplicated on upgrade

- **GIVEN** a database with 3 rows sharing the same `(merchant_id, amount_min, amount_max, currency, period_days)` and different `confidence` values (e.g. 0.7, 0.9, 0.5) and `last_seen_date` values
- **WHEN** `alembic upgrade head` is run
- **THEN** after upgrade, exactly 1 row exists per upsert-key group
- **AND** the surviving row is the one with the highest `confidence` (0.9 in the example)
- **AND** `last_seen_date` is the `max(last_seen_date)` across the group (preserves the freshest seen-date for tie-breaking)

#### Scenario: Concurrent inserts surface as IntegrityError

- **GIVEN** two `INSERT` statements on `recurring_rules` with the same upsert key arriving concurrently
- **WHEN** both reach the database
- **THEN** one succeeds and the other raises an `IntegrityError` (the unique constraint surfaces the race)
- **AND** the application layer's existing `IntegrityError` race guard (per the merchants service pattern) catches and retries the loser

#### Scenario: Downgrade drops the unique constraint

- **GIVEN** the unique constraint is in place
- **WHEN** `alembic downgrade -1` is run
- **THEN** the unique constraint `uq_recurring_rules_upsert_key` is dropped
- **AND** no data is lost (the deduped rows remain)

#### Scenario: Non-unique read-side index is preserved

- **GIVEN** the existing composite index `ix_recurring_rules_merchant_currency_period` (3-column, non-unique) is in place
- **WHEN** the unique-constraint migration runs
- **THEN** the 3-column composite index is NOT dropped (it still serves read-side queries that filter on `(merchant_id, currency, period_days)` alone, where the unique 5-column constraint is a wider index)

## Out of Scope

A `deactivated_at` timestamp column on `RecurringRule`; recurring override UI in the web layer (decision #12 — PATCH is the per-item override path); Phase 2 PR #6 (Docs + e2e); the `_metadata_completeness` cherry-pick (already in main); improving LLM installment detection (a future PR can improve the LLM; the `installment_number IS NULL` filter is the v1 path); recurring rule generation via LLM (the algorithm is deterministic + statistical); Sentry alerts for detector failures (decision #14: log only); rewriting the detector's SELECT-then-INSERT upsert as `INSERT ... ON CONFLICT` (the unique constraint surfaces the race as `IntegrityError` — the application layer handles it via the existing race-guard pattern; the SELECT path is preserved).
